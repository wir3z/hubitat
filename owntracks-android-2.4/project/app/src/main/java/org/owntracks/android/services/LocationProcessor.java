...
public class LocationProcessor {
    ...
    public void publishLocationMessage(@Nullable String trigger) {
        ...
        if (locationRepo.getCurrentPublishedLocation().getValue() == null) {
            Timber.e("no location available - sending empty message");
            // continue and send out a blank location message so that the settings can be sent to host
            message = MessageLocation.fromDefaults();
        } else {
            ...
            // check if we are under our accuracy limit
            if (ignoreLowAccuracy(currentLocation)) {
                // send out the message for a manual or ping message regardless
                if (MessageLocation.REPORT_TYPE_PING.equals(trigger) || MessageLocation.REPORT_TYPE_USER.equals(trigger)) {
                    // set the timestamp to "0" to indicate the location information is invalid
                    message.setTimestamp(0);
                } else {
                    // for all other reports, just return without sending
                    return;
                }
            }
        }
        ...
        if (preferences.getPubLocationExtendedData()) {
            if (wifiInfoProvider.isConnected()) {
                message.setSsid(wifiInfoProvider.getSSID());
                message.setBssid(wifiInfoProvider.getBSSID());
            }
        ...
        message.setWifistate(wifiInfoProvider.isWiFiEnabled());
        message.setPowerSave(deviceMetricsProvider.getPowerSave());
        message.setBatteryOptimizations(deviceMetricsProvider.getBatteryOptimizations());
        message.setAppHibernation(deviceMetricsProvider.getAppHibernation());
        message.setLocationPermission(deviceMetricsProvider.getLocationPermission());
